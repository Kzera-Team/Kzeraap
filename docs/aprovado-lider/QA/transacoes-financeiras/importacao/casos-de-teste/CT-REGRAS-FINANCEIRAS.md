...
Resultado obtido:
- confirmar() lancou "Pacote congelado foi alterado ou corrompido. Gere a previa novamente." mesmo em fluxo completo do zero
- artefatos nao foram criados: transacoes, pagamentos e movimentos oficiais permanecem zerados
- nenhum vinculo rastreavel foi gerado porque confirmar() falha antes de criar qualquer artefato

Causa raiz identificada (analise estatica):
- arquivo: src/infrastructure/repositories/ImportacaoStagingRepository.ts
- funcao: mergeTransacaoPayload (linha 53) e mergeFinanceiroPayload (linha 97)
- problema: registro.dadosNormalizados = payload.dadosNormalizados armazena referencia direta (sem structuredClone)
- logo apos o retorno de mergeTransacaoPayload, o bloco finally chama releaseObject(payload) (linha 141)
- releaseObject percorre recursivamente payload e seta TODAS as propriedades para undefined, inclusive as de payload.dadosNormalizados
- registro.dadosNormalizados aponta para o mesmo objeto zerando todas as propriedades (clienteNome, total, valorPago, custo, lucro, dataTransacao etc.)
- na assinatura (assinaturaPacoteDeItens): stableStringify({clienteNome: undefined, total: undefined, ...}) produz {"clienteNome":undefined,"total":undefined,...} porque ${JSON.stringify(undefined)} em template literal vira a string "undefined"
- em packJson (PayloadProvider.ts linha 11): JSON.stringify descarta propriedades com valor undefined
- ao recuperar o pacote do storage via unpackJson: JSON.parse produz {} (objeto vazio sem as propriedades)
- stableStringify({}) produz {} — diferente de {"clienteNome":undefined,"total":undefined,...}
- hashCurto diverge entre criacao e validacao: assinaturaAtual !== pacote.assinaturaPacote

Evidencias de codigo:
- src/infrastructure/repositories/ImportacaoStagingRepository.ts:53 (referencia direta sem clone)
- src/infrastructure/repositories/ImportacaoStagingRepository.ts:141 (releaseObject(payload) no finally)
- src/runtime/RuntimeCleanup.ts:20 (record[key] = undefined para cada chave recursivamente)
- src/runtime/PayloadProvider.ts:11 (JSON.stringify descarta undefined)
- src/runtime/PayloadProvider.ts:53 (JSON.parse nao restaura undefined)
- src/application/importacao/ConfirmarImportacaoHistoricaFinanceiraUseCase.ts:162-167 (stableStringify inclui undefined como string via template literal)
- src/application/importacao/ConfirmarImportacaoHistoricaFinanceiraUseCase.ts:560-561 (validarPacoteCongelado lanca o erro)

Payload congelado original (representacao): stagingSnapshot.dadosNormalizados com todas as propriedades undefined (objetos zerados pelo releaseObject)
Payload validado na confirmacao: stagingSnapshot.dadosNormalizados como {} (propriedades descartadas pelo JSON.stringify durante packJson)
Diferenca: objeto com N chaves com valor "undefined" vs objeto vazio sem chaves
...
Resultado obtido:
- nao foi possivel executar: ResolverPendenciaImportacaoUseCase de aprovacao em massa sem botao na UI

Evidencias:
- nenhum seletor de aprovacao em massa encontrado na tela de importacao
- caso nao e executavel sem binding de UI
