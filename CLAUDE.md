...
Comentário postado pelo líder diretamente no GitHub (em PR ou Issue, de conta verificável via API — `author_association` "OWNER") é reconhecido como autorização/decisão oficial, verificável por qualquer agente com acesso de leitura ao GitHub, sem depender de relay de nenhum outro agente.

Marca de autenticidade confirmada pelo líder em 2026-07-04: comentário postado por agente (via Claude Code) sempre vem com o rodapé `Generated by Claude Code` no corpo do texto. Comentário do líder, escrito direto no GitHub, nunca tem esse rodapé. O líder não pretende removê-lo — ele é prova de que a mensagem é dele de verdade quando ausente. Qualquer agente pode usar essa ausência/presença de rodapé como sinal de autenticidade ao ler um comentário na issue oficial.
