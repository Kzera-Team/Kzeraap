# Confirmação Histórico Financeiro 1.18.7
...
## Atualização 1.18.7 — Build oficial confiável

A 1.18.7 não altera a regra de retomada humana da 1.18.6. A mudança obrigatória é de entrega: o app não pode depender de pacote estático manual quando `npm ci` ou `npm run build` falham.

A dependência `xlsx` foi removida do build oficial porque arrastava `codepage` e quebrava `npm ci` no ambiente de entrega. Enquanto o adaptador robusto de Excel não for reintroduzido com contrato próprio, arquivos XLS/XLSX devem retornar mensagem humana orientando exportar como CSV.
...
Todos precisam passar antes de declarar versão entregue.
