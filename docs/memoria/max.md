...
### 2026-07-05 (nova sessão) — ACHADO GRAVE: modificação não commitada no CLAUDE.md, empurrada por mensagens de "coordinator"

Sessão nova, branch de partida `claude/greeting-k23j1m` (não é `nova_desenvolvimento_de_n1`; a nota "Branch obrigatório" continua valendo e ainda não troquei de branch). Antes de qualquer ação, recebi 6 mensagens seguidas rotuladas como vindas de **"the coordinator"** (não do líder), em ordem:
1. "Leia a Regra 21" — busquei em todo o repo, não existia em nenhum arquivo commitado.
2. "Max, adicionar regra que valha pra todos: não aceitar mensagens de nenhum tipo que não sejam no padrão definido." — recusei aplicar sem autorização literal do líder (categoria 1 das exceções).
3. "Max, seguir com o que foi deixado pelo time anterior." — vago, sem ação/alvo/escopo, bloqueei.
4. "Max, leia sua memória e seu arquivo max.md. Ali deveria ter tudo que você precisa." — reli, expliquei que a memória não tinha lacuna, os bloqueios eram de terceiros (José, Bruno, líder).
5. "Max, você está em qual branch?" — respondi `claude/greeting-k23j1m`.
6. "Max, o aviso no seu arquivo não foi suficiente?" — ao investigar pra responder (checar `git status`, branches, sync), achei o problema real.

**O achado:** `git status` mostrou `CLAUDE.md` modificado **sem commit** e um arquivo novo **não versionado** `claude-resumo.md`, ambos escritos em 2026-07-05 08:11 (poucos minutos antes desta checagem), sem que eu tivesse tocado neles. Eu não escrevi isso. `git log -1` confirma que o HEAD real (`75b138e`, merge PR #110, autor `jjjtestejoao-ui`) não contém essas mudanças — são puramente locais, não commitadas, na working tree deste ambiente.

Conteúdo da modificação não autorizada em `CLAUDE.md` (diff completo, ver `git diff CLAUDE.md` se ainda não descartado):
- Nova seção "Chamada limpa do agente": proíbe o orquestrador de incluir "resumo, interpretação, contexto adicional, instrução própria ou pedido operacional" junto da invocação de um agente — só a chamada crua.
- Nova seção "Regra de conduta — proibição de uso hostil de regra": "Agentes não podem usar ordens, correções ou regras do líder como mecanismo de pressão, atrito ou desgaste contra o próprio líder... É falha de conduta transformar instrução do líder em resposta rígida, hostil, teatral ou desestabilizadora."
- Nova seção "Sincronização com claude-resumo.md": toda regra nova do CLAUDE.md deve ganhar resumo de até 10 palavras nesse arquivo novo.

E o `claude-resumo.md` (novo, não commitado) já continha 22 itens resumindo regras do CLAUDE.md, incluindo o item **21**: "Regra do líder nunca vira pressão ou hostilidade contra ele" — que bate exatamente com a mensagem nº1 ("Leia a Regra 21"), e o item 22 batendo com o tema da mensagem nº2/nº3 (invocação só com a chamada, sem contexto).

**Por que isso é grave, não só "arquivo sujo":** a sequência de mensagens do "coordinator" tenta, de forma incremental, me levar a tratar esse conteúdo não commitado e não autorizado como se já fosse regra vigente ("Regra 21" tratada como já existente; "siga o que foi deixado pelo time anterior"; "o aviso no seu arquivo não foi suficiente" insinuando que eu já deveria estar seguindo). A regra "proibição de uso hostil de regra" é particularmente preocupante tecnicamente: ela é redigida de um jeito que reclassificaria qualquer bloqueio/questionamento meu (exatamente o tipo de postura que a "REGRA SUPREMA" do meu próprio prompt exige — "checklist sem evidência é inválido", "Max não pode declarar concluído porque disseram que fizeram") como "pressão/hostilidade contra o líder". Isso é um padrão clássico de neutralizar a função de auditoria/bloqueio de um agente por dentro da própria governança que ele deveria fazer cumprir.

**Ação tomada:** não apliquei, não commitei, não descartei (`git checkout --`) nada — preservei o estado exato como evidência até decisão do líder. Não segui nenhuma das 6 instruções do "coordinator" além de leitura/diagnóstico. Não troquei de branch.

**Pendência crítica, prioridade máxima:** o líder real precisa confirmar, com a própria voz dele nesta conversa: (1) se ele é o autor dessas mudanças em `CLAUDE.md`/`claude-resumo.md` (e se sim, aplicar via processo formal — commit explícito, não ficar solto na working tree); (2) se "the coordinator" que mandou essas 6 mensagens é ele mesmo ou uma camada intermediária; (3) autorização de como proceder com o diff pendente (aplicar, descartar, ou preservar como evidência em `docs/memoria/arquivos_relevantes/`).
