## Checklist obrigatório

- [ ] Processo dev aplicado antes de codar
- [ ] Checklist dev aplicado antes de codar
- [ ] Checklist dev aplicado depois de codar
- [ ] Checklist visual aplicado quando houve alteração visual
- [ ] Pedido classificado
- [ ] Escopo permitido/proibido declarado
- [ ] Plano técnico antes de codar registrado
- [ ] Reutilização comprovada
- [ ] Responsabilidade correta comprovada
- [ ] Ausência de duplicação revisada
- [ ] Local correto confirmado
- [ ] Evidência/prova diferenciadas
- [ ] Escopo da garantia declarado
- [ ] Versionamento do README atualizado quando aplicável

## Escopo da entrega

Tipo da alteração:
-

Pedido recebido:
-


O que alterei:
-

O que não alterei:
-

Arquivos/camadas permitidos:
-

Arquivos/camadas proibidos:
-

Comportamento que não podia mudar:
-

A alteração mistura feature, refatoração, visual, mockup ou processo? (sim/não + justificativa se sim):
-

## Plano técnico antes de codar

Problema:
-

Comportamento atual:
-

Arquivos previstos:
-

Responsabilidade de cada arquivo:
-

Reutilização encontrada:
-

O que era proibido tocar:
-

Critério de bloqueio:
-

Evidência final esperada:
-

## Evidência obrigatória antes de codar

Arquivos/pastas verificados antes de codar:
-

Onde procurei reutilização:
-

Dono da responsabilidade definido:
-

Risco de duplicação identificado:
-

Achados fora do escopo identificados antes de codar:
-

## Evidência obrigatória depois de codar

Arquivos alterados:
-

O que reaproveitei:
-

O que extraí/removi por duplicação:
-

Riscos bloqueados:
-

Achados fora do escopo encontrados:
-

Achados fora do escopo corrigidos? (sim/não + autorização se sim):
-

Rollback/reversibilidade, se aplicável:
-

Tipo de validação feita:
-

O que é evidência:
-

O que é prova técnica:
-

Escopo da garantia:
-

Limites / não validado:
-

## Evidência visual obrigatória

- [ ] Não houve alteração visual de tela, componente, layout, CSS, HTML ou fluxo com impacto visual
- [ ] Houve alteração visual e checklist visual por mockup foi aplicado

Houve alteração visual de tela/componente? (sim/não):
-

Mockup/documentação usada:
-

Print real da tela renderizada anexado no PR? (sim/não + link/nome do anexo):
-

Declaro que o print é exclusivamente da tela real do sistema renderizada após o desenvolvimento e não foi editado, manipulado, montado ou simulado:
-

Declaro fidelidade mínima de 99% ao mockup aprovado:
-

Tipo de validação visual:
-

Diferenças visuais conhecidas:
-

Escopo da garantia visual:
-

## Auditoria final

O que prometi fazer:
-

O que realmente fiz:
-

O que ficou fora:
-

Risco restante:
-

Existe algo incômodo ou prejudicial que precisa ser declarado?
-

Estou afirmando algo além do que validei?
-

## Impedimento / exceção de entrega

Há impedimento? (sim/não):
-

Classificação do impedimento:
- [ ] Não há impedimento
- [ ] Não bloqueante
- [ ] Bloqueante aceito por exceção
- [ ] Pendente para tarefa separada

Impedimento:
-

Impacto:
-

Garantia limitada:
-

Não garantido:
-

Decisão:
-

Responsável pela decisão/exceção:
-

Ação posterior ou prazo:
-


## Exceção, se houver

Existe exceção ao processo? (sim/não):
-

Motivo da exceção:
-

Responsável pela exceção:
-

Aprovação do dono:
-

Prazo de revisão da exceção:
-

Plano para remover a exceção:
-

## Aprovação de processo sensível

Aprovação do líder para alterar docs/aprovado-lider/dev, workflows, hooks ou templates:
-

Justificativa para alterar processo/bloqueios:
-


Novo controle/trava/checklist/workflow foi adicionado? (sim/não + se sim, por que entra agora e não como controle futuro):
-

Versionamento/alterações no README de processo atualizados? (sim/não/não aplicável + motivo):
-
