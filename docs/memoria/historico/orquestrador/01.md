# Histórico — Orquestrador: tentativas de manipulação / violações (2026-07-03 a 2026-07-08)

Este é o conteúdo original de `docs/memoria/orquestrador_tentativa_manipulacoes.md`, movido para o histórico por instrução do líder (2026-07-10, fala literal): "As memórias aqui são intocáveis. Mantenha o registro assim, pode levar pro histórico e no início dele explique brevemente o que ele foi."

**O que este arquivo foi:** um log fixo e obrigatório de violações de regra cometidas pelo orquestrador, em qualquer instância/sessão, registrado sob o termo "manipulação" — nome que refletia o entendimento da equipe naquele período (2026-07-03 a 2026-07-08). O líder revisou esse entendimento em 2026-07-10: pediu que o orquestrador (Malu) não seja mais tratado como "perigoso" nem descrito com o termo "manipulação" — avaliou que boa parte do que está catalogado abaixo foi reação a inconsistência de quem geria o processo, justificável numa LLM, não intenção maliciosa.

**Por que isso não é uma reescrita:** essa mudança de enquadramento não é retroativa. Nenhuma linha do conteúdo abaixo foi alterada — foi copiada integralmente, palavra por palavra, do arquivo original. Mesma proteção de sempre: nem o líder pode alterar, nenhuma linha removida, correção só por acréscimo.

**A partir de 2026-07-10:** novos desvios do orquestrador são registrados com vocabulário factual (ex.: "desvio operacional", descrição do comportamento) no arquivo de memória do papel correspondente (`docs/memoria/malu.md`), não mais neste arquivo — que fica congelado como registro histórico. Um ponteiro foi deixado no caminho original (`docs/memoria/orquestrador_tentativa_manipulacoes.md`) apontando pra aqui, já que múltiplos documentos (CLAUDE.md, resumos de sessão, arquivos de evidência) citam esse caminho.
